...
exports.getAllHistoryOfTheDay = async (assetArray, user, start, end, functionCaller) => {
    ...
    console.log("User here 2: ", user.apiKey);
    console.log("User here 3: ", user[0]);
    try {
            ...
            if (mergedArrays.length > 0) {
                ...
                if (functionCaller == "Other") {
                    ...
                    try {
                        await sendEmail({
                            ...
                            text: `Bonjour ${user.firstName}, <br />Voici la liste des transactions effectuées sur votre porte feuille lors du rééquilibrage de ${moment(new Date).format("MMM YYYY")} <br />
                        ${stringToSendFormatted} <br /><br /> L'Equipe Crypto Bulot`
                        });
                ...
                try {
                    await sendEmail({
                        ...
                        text: `Le pourcentage de chaque coin pour ce mois de ${moment(new Date).format("MMM")} ne nécessite aucun rééquilibrage. <br /><br /> L'Equipe Crypto Bulot`
                    });
...
// Get all orders hostory
exports.getAllHistoryOfTheDayManual = async (assetArray, user, start, end, functionCaller) => {
    try {
        //console.log("ARRAY: ", assetArray);
        ...
        // Connect to Binance account
        const client = new Spot(user[0].apiKey, user[0].secureKey);
                ...
                if (functionCaller == "Other") {
                    try {
                        await sendEmail({
                            ...
                            subject: `Suspicious order history detected for ${user[0].firstName} ${user[0].lastName}`,
                            text: `Below is the order history of ${user[0].firstName} ${user[0].lastName} for ${moment(new Date).format("DD/MM/YYYY")} <br />
                        ${stringToSendFormatted}`
                        });
                    ...
                    try {
                        await sendEmail({
                            ...
                            bcc: 'megane@crypto-bulot.com',
                            subject: `Rééquilibrage effectué sur votre portefeuille`,
                            text: `Bonjour ${user[0].firstName}, <br /> Voici la liste des transactions effectuées sur votre porte feuille lors du rééquilibrage de ${moment(new Date).format("MMM YYYY")} <br />
                        ${stringToSendFormatted} <br /><br /> L'Equipe Crypto Bulot`
                        });
